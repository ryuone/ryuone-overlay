...
SRC_URI="http://s3.amazonaws.com/downloads.basho.com/riak/${MAJ_PV}.${MED_PV}/${MAJ_PV}.${MED_PV}.${MIN_PV}/${P}.tar.gz"
...
src_install() {
	...
	dosbin rel/riak/bin/riak rel/riak/bin/riak-admin rel/riak/bin/search-cmd || die

	# install /etc/riak stuff
	insinto /etc/${PN}
	doins rel/riak/etc/* || die

	# create neccessary directories
	keepdir /var/lib/${PN}/{bitcask,ring}
	keepdir /var/log/${PN}/sasl
	keepdir /var/run/${PN}
	fowners riak:riak /var/{lib,log,run}/${PN} || die
	fowners riak:riak /var/lib/${PN}/{bitcask,ring}  || die
	fowners riak:riak /var/log/${PN}/sasl  || die
